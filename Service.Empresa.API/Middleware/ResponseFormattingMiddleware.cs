        ...
        try
        {
            ...
            if (string.IsNullOrWhiteSpace(body))
            {
                if (context.Response.StatusCode == 204)
                {
                    ...
                    var responseMessages = context.Items.TryGetValue("ResponseMessages", out var responseMsgObj) && responseMsgObj is List<string> responseMsgList
                        ? responseMsgList
                        : [];
                    var emptyApiResponse = ApiResponse<object>.OkNull(responseMessages);
